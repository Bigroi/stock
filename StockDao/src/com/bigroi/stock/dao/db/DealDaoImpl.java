...
public class DealDaoImpl implements DealDao {
    ...
    private static final String GET_POSSIBLE_BIDS =
            "SELECT "
    ...
    private static final String SET_SELLER_STATUS =
            " UPDATE DEAL "
                    + " SET SELLER_CHOICE = ?, BUYER_ALERT = ? "
                    + " WHERE ID = ? ";

    private static final String SET_BUYER_STATUS =
            " UPDATE DEAL "
                    + " SET BUYER_CHOICE = ?, SELLER_ALERT = ? "
                    + " WHERE ID = ? ";
    ...
    private static final String RESET_SELLER_ALTERS =
            "UPDATE DEAL "
                    + " SET SELLER_ALERT = 'N' "
                    + " WHERE SELLER_ADDRESS_ID IN (SELECT ID FROM ADDRESS WHERE COMPANY_ID = ?)";

    private static final String RESET_BUYER_ALTERS =
            "UPDATE DEAL "
                    + " SET BUYER_ALERT = 'N' "
                    + " WHERE BUYER_ADDRESS_ID IN (SELECT ID FROM ADDRESS WHERE COMPANY_ID = ?)";
    ...
    public void getTestPossibleDeals(List<TradeLot> tradeLots, List<TradeTender> tradeTenders, long productId, String sessionId) {
        var sql = GET_POSSIBLE_BIDS
                .replace("COMPANY_CONDITION", "")
    ...
    @Override
    public void resetAlerts(long companyId) {
        var template = new JdbcTemplate(datasource);
        template.update(RESET_SELLER_ALTERS, companyId);
        template.update(RESET_BUYER_ALTERS, companyId);
    }

    @Override
    public void getPossibleDeals(List<TradeLot> tradeLots, List<TradeTender> tradeTenders, long productId) {
        String sql = GET_POSSIBLE_BIDS
                .replace("COMPANY_CONDITION", "AND T.COMPANY_ID <> L.COMPANY_ID ")
    ...
    public void setBuyerStatus(Deal deal) {
        JdbcTemplate jdbcTemplate = new JdbcTemplate(datasource);
        jdbcTemplate.update(SET_BUYER_STATUS, deal.getBuyerChoice().getCode(), deal.getSellerAlert(), deal.getId());
    }

    @Override
    public void setSellerStatus(Deal deal) {
        JdbcTemplate jdbcTemplate = new JdbcTemplate(datasource);
        jdbcTemplate.update(SET_SELLER_STATUS, deal.getSellerChoice().getCode(), deal.getBuyerAlert(), deal.getId());
    }
    ...
    private static final class DealRowMapper implements RowMapper<Deal> {
        ...
        public Deal mapRow(ResultSet rs, int rowNum) throws SQLException {
            ...
            deal.setBuyerAlert(rs.getString("BUYER_ALERT"));
            deal.setSellerAlert(rs.getString("SELLER_ALERT"));
